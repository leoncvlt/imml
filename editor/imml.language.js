...
const language = {
  ...
  tokenizer: {
    root: [
      // headers (with #)
      // edited to allow capturing regardless of indentation
      [/^(\s{0,})(#+)((?:[^\\#]|@escapes)+)((?:#+)?)/, ["white", "keyword", "keyword", "keyword"]],
      ...
      // edited out since indentation in imml is used all over the place
      // [/^(\t|[ ]{4})[^ ].*$/, 'string'],
...
const createTheme = (base) => ({
  base,
  inherit: true,
  rules: [{ token: "imml.page", foreground: "d45434", fontStyle: "bold" }],
  colors: {
