...
export const transform = (nodes, level = 0) => {
  const stack = [];
...
export const process = (source, page = null, data = null) => {
  ...
  for (const node of source) {
    ...
    if (isObject(node)) {
      const newPage = Object.keys(node)[0].replace(/[\[\]']+/g, "");
...
export const render = (data, document, root) => {
