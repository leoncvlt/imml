...
const transform = (nodes, level = 0, raw = false) => {
  const stack = [];
  for (let i = 0; i < nodes.length; i++) {
    const node = nodes[i];
    const nextNode = i < nodes.length - 1 ? nodes[i + 1] : { indentation: -1 };
    const value = raw ? node.rawValue : node.value;
...
const parseOption = (option) => {
  ...
  if (isObject(option)) {
    ...
    value = tokens[tokens.length - 1].replaceAll(NEWLINE, "").trim();
  }
...
const process = (source, page = null, data = null) => {
  ...
  for (const node of source) {
    ...
    if (isObject(node)) {
      const newPage = Object.keys(node)[0].replace(BLOCK_CLEAN_PATTERN, "");
      const newPath = (page ? data.pages[page].path : "") + "/" + newPage;
      ...
      process(Object.values(node)[0], newPage, data);
      continue;
    }
    ...
    if (isString(node)) {
      if (page) {
        const value = node.replace(NEWLINE, "\n");
  ...
  Object.entries(data.pages).forEach(([pageId, pageData]) => {
    ...
    elements.forEach((element) => {
      switch (element.type) {
        ...
        case "text":
          const text = marked(element.value, { renderer: markdownRenderer });
...
export { preprocess, transform, process, render };
