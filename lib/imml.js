...
export const tokenize = (data) => {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const level = value.trim() ? currentLine.search(/\S|$/) : previousToken.level;
    let token = {
      level,
      path: paths[level] || "",
    };
    ...
    if (
      ...
      result[previousToken.__index] = {
        level: previousToken.level,
        head: token.level,
        content: portalText,
        link: portalText,
        type: "portal",
        visible: true,
        path: paths[previousToken.level] || "",
      };
      token = {
        ...token,
        path: newPath,
...
export const parse = (tokens) => {
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    const currentPage = pathBits[pathBits.length - 1] || HOME_ID;
    if (!data.pages[currentPage]) {
      data = {
        ...data,
        pages: {
          ...data.pages,
          [currentPage]: { elements: [], path: token.path },
        },
      };
      if (isFieldTrueish(data.options["show-pages-title"])) {
        data.pages[currentPage].elements.push({
          type: "title",
          text: currentPage === HOME_ID ? data.options["title"] || "Home" : currentPage,
        });
      }
    }
...
export const render = (data, document, root = null) => {
  ...
  Object.entries(data.pages).forEach(([pageId, pageData]) => {
    ...
    slugs.forEach((slug, index) => {
      ...
      if (isLastSlug) {
        breadCrumb.href = `#${slug === "home" ? "" : slug}`;
  ...
  const homePage = pages.querySelector(`#${HOME_ID}`);
